...
{
	public:
		TrashHolder(uint16_t type, MagicEffect_t _effect = CONST_ME_NONE): Item(type), effect(_effect) {}
		virtual ~TrashHolder() {}
		...
		virtual ReturnValue __queryAdd(int32_t, const Thing*, uint32_t,
			uint32_t, Creature* = NULL) const {return RETURNVALUE_NOERROR;}
		virtual ReturnValue __queryMaxCount(int32_t, const Thing*, uint32_t,
			uint32_t&, uint32_t) const {return RETURNVALUE_NOERROR;}
