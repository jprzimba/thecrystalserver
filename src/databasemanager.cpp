...
uint32_t DatabaseManager::updateDatabase()
{
	...
	switch(version)
	{
		case 0:
		{
			std::clog << "Updating database to version: 1 (initial database)." << std::endl;
		...
		case 2:
		{
			...
			{
				query << "CREATE TABLE IF NOT EXISTS `new_player_autoloot` ("
					"`id` INTEGER PRIMARY KEY AUTOINCREMENT, "
					"`player_id` INTEGER NOT NULL, "
					"`itemid` INTEGER NOT NULL, "
				...
				query.str("");
				query << "INSERT INTO `new_player_autoloot` (`id`, `player_id`, `itemid`, `containerid`) "
					"SELECT `id`, `player_id`, `itemid`, 0 FROM `player_autoloot`;";
				db->query(query.str());

				query.str("");
				query << "DROP TABLE `player_autoloot`;";
				db->query(query.str());
		
				query.str("");
				query << "ALTER TABLE `new_player_autoloot` RENAME TO `player_autoloot`;";
			}
