...
void ProtocolHTTP::onRecvFirstMessage(NetworkMessage&)
{
	if(OutputMessage_ptr output = OutputMessagePool::getInstance()->getOutputMessage(this, false))
	{
		...
		output->putString("Server: The Crystal Server httpd/0.4\r\n");
		...
		output->putString("\r\n");
		output->putString("<html><head><title>The Crystal Server httpd</title></head><body>It works (apache ripoff ;D)!</body></html>");
