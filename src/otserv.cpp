...
bool argumentsHandler(StringVec args)
{
	...
	for(StringVec::iterator it = args.begin(); it != args.end(); ++it)
	{
		...
		if((*it) == "--version" || (*it) == "-v")
		{
			...
			"Visit our GitHub for updates, support and resources: https://github.com/jprzimba/thecrystalserver.\n";
			return false;
		}
...
void otserv(StringVec, ServiceManager* services)
{
	...
	std::clog << SOFTWARE_NAME << ", version " << SOFTWARE_VERSION << " (" << SOFTWARE_CODENAME << ")" << std::endl
		...
		<< "Visit our GitHub for updates, support and resources: https://github.com/jprzimba/thecrystalserver." << std::endl << std::endl;
	std::stringstream ss;
#ifdef __DEBUG__
	ss << " GLOBAL";
#endif
